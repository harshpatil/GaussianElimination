...
main(int argc, char** argv) {
    ...
    /* destination address is set to highest ranked procees (master) **/
    dest = p -1;
 ...
 * Highest ranked processor will act as master processor and get data from all the other processes
 */
    if (my_rank == (p-1)) {
    ...
    /*  Master processor printing the result
     */
    if (my_rank == (p-1)) {
...
/* If it is master process it needs to send data to slave processes **/
    if (my_rank == (p-1)){
