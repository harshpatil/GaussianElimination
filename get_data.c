...
main(int argc, char** argv) {
    ...
    /* [] Setting destination address to the highest ranked process **/
    dest = p -1;
...
/* Add up the integrals calculated by each process */
    ...
    if (my_rank == (p-1)) { /*[] Highest ranked processor will collect data from all other processes and calculate the total**/

        total = integral;

        for (source = 0; source < p-1; source++) { /**[]Recieving data from all other processes*/
    ...
    if (my_rank == (p-1)) { /*[]Highest ranked process is outputting the result**/
...
void Get_data(
    ...
    /*[] Setting source to the highes rank process, since he will be sending data to other processes**/
    ...
    if (my_rank == (p-1)){ /*[] If this is the highest rank process it needs to send data of the program input variables to other processes **/

        *a_ptr = A;
        *b_ptr = B;
        *n_ptr = N;

        for (dest = 0; dest < p-1; dest++){ /**[]Sending data to all other processes apart from highest ranked process.*/
